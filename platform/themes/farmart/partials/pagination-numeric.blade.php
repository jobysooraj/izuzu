...
<div class="pagination-numeric-short">
    ...
    <form class="toolbar-pagination" action="{{ $paginator->path() }}" method="GET">
        ...
        @endforeach --}}
        @foreach (request()->input() as $key => $item)
        @if (!in_array($key, [$paginator->getPageName(), 'vin', 'sp']) && is_string($item))
        <input name="{{ $key }}" type="hidden" value="{{ $item }}">
        @endif
        @endforeach

        <input class="catalog-page-number" name="{{ $paginator->getPageName() }}" type="number" value="{{ $paginator->currentPage() }}" min="1" max="{{ $paginator->lastPage() }}" step="1">
